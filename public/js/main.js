$("a.menu_switch").click(function(evt) {
  ...
  if ($(this).attr("data-menu-status") == "false") {
    ...
    if (getCookie("menu_opened") == "") {
      setCookie("menu_opened", true, 180, "mvc.dev");
    }
  } else {
    ...
    deleteCookie("menu_opened", "mvc.dev");
  }
});

$("form.appForm input:not(.no_float)")
  .on("focus", function() {
    $(this)
      .parent()
      .find("label")
...
$(
  "div.radio_button, div.checkbox_button, label.radio span, label.checkbox span, a.language_switch.user"
).click(function(evt) {
  evt.stopPropagation();
});
...
(function() {
  ...
  for (var i = 0, ii = closeMessageButtons.length; i < ii; i++) {
    closeMessageButtons[i].addEventListener(
      "click",
      function(evt) {
        evt.preventDefault();
